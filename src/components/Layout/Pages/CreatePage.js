...
const CreatePage = ({ selectedItems, updateSelectedItems }) => {
  ...
  const renderLibraryContent = () => {
    return selectedItems.map((item, index) => (
      <div key={`${item.type}-${index}`} class='dropdown' className={styles.libraryItem}>
        <h2>{`${item.type.charAt(0).toUpperCase() + item.type.slice(1)}`}</h2>
        {renderLibraryImage(item.type)}
        <div class='dropdown-content' className={styles.itemOptions}>
          <p><span onClick={() => handleEditItem(item)}>Edit</span></p>
          <p><span onClick={() => handleShareItem(item)}>Share</span></p>
          <p><span onClick={() => handleDeleteItem(item)}>Delete</span></p>
        </div>
      </div>
    ));
  };
